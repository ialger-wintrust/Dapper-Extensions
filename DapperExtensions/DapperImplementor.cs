...
namespace DapperExtensions
{
    public interface IDapperImplementor
    {
        ...
        bool Delete<T>(IDbConnection connection, IEnumerable<T>? entities, IDbTransaction? transaction, int? commandTimeout);
    ...
    public class DapperImplementor : IDapperImplementor
    {
        ...
        public int Count<T>(IDbConnection connection, object? predicate, IDbTransaction? transaction, int? commandTimeout, IList<IReferenceMap> includedProperties = null)
        {
            ...
            var results = connection.ExecuteScalar<int>(sql, dynamicParameters, transaction, commandTimeout, CommandType.Text);
            return results;
        }
        ...
        public T? Get<T>(IDbConnection connection, dynamic? id, IDbTransaction? transaction, int? commandTimeout, IList<IReferenceMap>? includedProperties = null)
        {
            ...
            var parameters = new Dictionary<string, object>();
            string sql = SqlGenerator.Select(classMap, predicate, null, parameters, null, includedProperties);
            var dynamicParameters = GetDynamicParameters(parameters);
        ...
        protected static IPredicate? GetEntityPredicate(IClassMapper classMap, object? entity)
        {
            ...
            foreach (var kvp in keyValuePairs)
            {
                ...
                ArgumentNullException.ThrowIfNull(fieldPredicate, nameof(fieldPredicate));
